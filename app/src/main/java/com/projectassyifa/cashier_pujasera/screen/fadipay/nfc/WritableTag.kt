...
class WritableTag @Throws(FormatException::class) constructor(tag: Tag) {
    ...
    val tagId: String?
        get() {
            if (ndef != null) {
                return bytesToHexString(ndef.tag.id)?.toLong(radix = 16).toString()
            } else if (ndefFormatable != null) {
                return bytesToHexString(ndefFormatable.tag.id)?.toLong(radix = 16).toString()
            }
            return null
        }
    ...
    companion object {
        fun bytesToHexString(src: ByteArray): String? {
            ...
            for (b in src) {
                ...
                Log.i("coba", String.format("%02X", b))
            }
            //Log.i("coba", sb.reverse().toString())
